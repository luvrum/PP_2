...
class Coin(pygame.sprite.Sprite):
    '''
    копия енеми только для монетки
    '''
    def __init__(self):
        super().__init__()
    ...
    def contact(self):
        '''
        телепортирует монетку наверх и добавляет в счетчик
        '''
        global COINS, SPEED
        SPEED+=1
        COINS+=1
...
while True:
    ...
    if pygame.sprite.spritecollideany(P1, friends):
        '''
        столкновение с монеткой вызывает метод ее класса, уничтожает текущий экземпляр
        создает новый экземпляр и доабвляет его в группы
        '''
        C1.contact()
        for entity in friends:
            entity.kill()
        C1 = random.choice([Coin, Coin_big])()
        friends.add(C1)
        all_sprites.add(C1)
