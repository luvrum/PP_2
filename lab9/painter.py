...
pygame.draw.line(screen, pygame.Color('black'), (0, 31), (WINDOW_WIDTH, 31), 5)   # splitting line

while running:
    pygame.draw.rect(screen, GREEN, (0, 0, WINDOW_WIDTH, 30))  # updating status
    ...
    for event in pygame.event.get():
        ...
        if shape == 'line' or shape == 'eraser':  # line and erasor have approximate same algorithm
            ...
            if prev:
                    ...
                    prev = cur #previous point equals current point
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if shape == 'circle':
                    ...
                    x = min(prev[0], cur[0])  #minimal coordinate
                    y = min(prev[1], cur[1])
                    lx = abs(prev[0]-cur[0])  # length of rectangle
                    ly = abs(prev[1]-cur[1])
